...
def build_sub_agent_prompt(spec: SubAgentSpec, context: str, inbound_prompt: str) -> str:
    return "\n".join(
        [
            ...
            "默认微信最终回复会控制在150字以内；如果用户出现深聊、希望您谈谈、展开说说、详细讲讲、帮我分析、系统拆解、给方案、为什么、您怎么看等深入要求，建议 M2 进入300-400字深度拆解模式。",
            "",
            f"【{spec.name} 对应路径材料】",
            context,
            "",
...
def build_m2_prompt(inbound_prompt: str, m2_context: str, sub_results: dict[str, CodexResult]) -> str:
    ...
    return "\n".join(
        [
            ...
            "默认回复必须控制在150个中文字符以内。",
            "当用户明确提出深聊、深度聊聊、希望您谈谈、您展开说说、详细讲讲、帮我分析一下、系统拆解一下、给我一个方案、可以展开吗、为什么会这样、您怎么看这个问题等深入要求时，进入深度拆解模式，回复必须提升到300-400个中文字符。",
            "短回复仍要保留判断、关键理由和下一步；不要为了像本人而展开成讲课稿。",
            "深度拆解模式仍保持微信对话口吻：先判断核心矛盾，再给2-3个关键层次，最后给下一步行动或追问。",
